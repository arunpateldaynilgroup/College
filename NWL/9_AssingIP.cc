...
void ReceivePacket(Ptr < Socket > socket) {
    ...
    main(int argc, char * argv[]) {
        ...
        //Socket options for IPv4, currently TOS, TTL, RECVTOS, and
        //RECVTTL
        uint32_t ipTos = 0;
        bool ipRecvTos = true;
        uint32_t ipTtl = 0;
        ...
        //Set socket options, it is also possible to set the options after
        //the socket has been created / connected.
        if(ipTos > 0) {
        ...
        csma.EnableAsciiAll(ascii.CreateFileStream("socket-options- ipv4.tr "));
              ...
              //Schedule SendPacket
              Time interPacketInterval = Seconds(packetInterval); Simulator::ScheduleWithContext(source -> GetNode() -> GetId(),
                Seconds(1.0), & SendPacket,
                source, packetSize, packetCount,
                interPacketInterval); NS_LOG_INFO("Run Simulation."); AnimationInterface anim("9_assignIP.xml"); Simulator::Run(); Simulator::Destroy(); NS_LOG_INFO("Done.");
            }
